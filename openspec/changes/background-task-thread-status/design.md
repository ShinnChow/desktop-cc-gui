...
- **D13 rename 链路随迁（2026-08-28 review 发现，P1）**：pending→final rename（自动标题完成等常见路径）走 `renameThreadStateIdentity`，其 status 合并是显式逐字段枚举、缺 `backgroundTaskRunningCount`——newThreadId 已有 status 时合并丢计数；且 `backgroundTaskStore` 的 key 是 threadId、不随 rename 迁移，迁移后 watcher 枚举 / sink 回写 / 时间线 upsert 全挂旧 id，终态写进幽灵条目，新会话卡片与 pill 永不更新。修复：① 合并补 `backgroundTaskRunningCount: Math.max(old, existing ?? 0)`（两侧 live 任务都不缩水）；② store 新增 `renameBackgroundTasksForThread`（幂等、旧表不存在 no-op、合并只补缺、emitChange 一次驱动 sync 重算两侧计数）；③ `renameThreadId` case 与 ensureThread pending-rename inline 分支各接一行迁移调用（沿用 `scheduleRenameTurnFinalMetaThreadId` 同款 reducer 内副作用模式）。
- **D14 卡片权威快照直读（2026-08-28 真机「并行双 pi 会话」事故防御）**：并行场景磁盘取证——两任务 receipt 正常、9 秒后被 kill（第二 resident 创建时触发，pi 扩展行为，前端不可控），registry 正确写 `killed`、**被 kill 任务不发 notification**、store 经 watcher 正确翻终态（pill 6 total 格式），但时间线卡片永停「运行中」。全真实管线集成测试（真实磁盘 killed JSON + 真 store + 真 buildConversationItem + 真 reducer merge + 折叠条渲染）证明逻辑层完备 → 缺口在运行时特定条件（双 resident 事件交错），纸面无法收敛。防御性修复：卡片状态从「时间线投影」解耦为「store 权威快照直读」——`useBackgroundTaskLiveSnapshot`（useSyncExternalStore 版本号订阅，按 itemId/toolId 匹配）+ `canonicalBackgroundTaskFromRecord` 转换，`ToolBlockRenderer` backgroundTask 分支 store live 快照优先、output 快照兜底。时间线更新丢失类 bug 全部自愈；非当前会话/未 hydrate 时自然回落 output。
- **Review 遗留低危记录（接受不改）**：① `removeThread`/rename 后 sync 对已消失 key 补 0 dispatch 会在 `threadStatusById` 重建全默认条目（幽灵残留，无 summary/sidebar 引用，纯内存）；② watcher `deadSince` key 为裸 taskId，无 workspace 前缀，跨会话 9-hex 撞名概率极低；③ watcher 3s interval 与 8-26 原实现等价，仅状态变化时 dispatch，无秒级 setState 轮询，不触 Render Perf 红线；④ topbar tabs 经确认无 `thread-runtime-dot` 渲染点，投影第四位类型兼容无 UI 影响（D6 成立）。
