...
- [`2026-08-24-fix-shared-session-recovery-exit-closure`](2026-08-24-fix-shared-session-recovery-exit-closure/proposal.md) — verified implementation，已同步 `shared-session-recovery-exit`；Recovery Exit Ladder（Probe/Stop/停止并重建/durable 放弃本轮）落地，`shared_session_v2_abandon_unresolved_attempt` 写 durable cancel；`target-unavailable` 细化语义已手工并入 `shared-send-pipeline` 主 spec；基石 ADR 已含校准
- [`2026-08-24-simplify-shared-recovery-bar-ux`](2026-08-24-simplify-shared-recovery-bar-ux/proposal.md) — verified implementation，已同步 `shared-recovery-bar-ux` / `shared-session-recovery-exit`；recovery 条收敛单行两主按钮 + 自动处理梯子，应用内 ConfirmDialog 替代 `window.confirm`（WKWebView 静默失败 / WebView2 卡死实证）
- [`2026-08-24-fix-shared-context-resume-integrity`](2026-08-24-fix-shared-context-resume-integrity/proposal.md) — verified implementation，已同步 `shared-context-resume-integrity`、`shared-context-compiler` 等 5 capability；binding `nativeContextTrust` + dirty 条件 rematerialize + `empty-context-handoff` 一等错误类别；用户验收通过
- [`2026-08-24-fix-shared-codex-context-projection`](2026-08-24-fix-shared-codex-context-projection/proposal.md) — verified implementation，已同步 `shared-context-compiler` / `shared-context-delivery` / `shared-provider-retry`；Shared Codex context 改走 portable transcript（关 structured import/tool history），缺 reasoning item 判 permanent `config` 禁同 Binding 重试；基石 ADR 校准已回写
- [`2026-08-24-fix-shared-create-default-provider-catalog`](2026-08-24-fix-shared-create-default-provider-catalog/proposal.md) — verified implementation，已同步 `shared-execution-target` / `shared-session-engine-selection`；Shared 创建默认 Provider 改有序列表第一项 + profile 权威 catalog/mapping，打开既有会话回显 last target；人工验收通过；基石 ADR 校准已回写
- [`2026-08-24-fix-shared-model-picker-display-authority`](2026-08-24-fix-shared-model-picker-display-authority/proposal.md) — verified implementation，已同步 `shared-execution-target`；Shared Atomic 闭合态以 `selectedNextTarget` 快照为 display authority，禁止全局 model 回落串台
- [`2026-08-24-fix-shared-atomic-model-reasoning-linkage`](2026-08-24-fix-shared-atomic-model-reasoning-linkage/proposal.md) — verified implementation，已同步 `shared-execution-target`；reasoning options/effort 由 target 模型 capability 驱动，禁止全局 `activeEngine` 档位冒充
- [`2026-08-24-fix-shared-user-image-bubble-projection`](2026-08-24-fix-shared-user-image-bubble-projection/proposal.md) — verified implementation，已同步 `shared-user-image-canvas-projection`、`shared-canonical-projection` 等 3 capability；turnRequested 用户消息携带 image locators，Shared 历史不丢图
- [`2026-08-24-enhance-shared-subagent-parent-id-attach`](2026-08-24-enhance-shared-subagent-parent-id-attach/proposal.md) — verified implementation，已同步 `subagent-session-tree-navigation`（delta 内容已在主 spec，archive --skip-specs）；Shared 子代理 parent remap 到 `shared:` owner，hide 后不再升顶层「下崽」
- [`2026-08-24-add-shared-squad-control-plane`](2026-08-24-add-shared-squad-control-plane/proposal.md) — verified implementation，已同步 `shared-squad-control-plane`、`shared-canonical-projection` 等 3 capability；Canonical event log 新增 Squad typed orchestration facts，Lead 只提 DAG、mossx validator 决定执行边界
- [`2026-08-24-add-shared-session-project-memory-capture`](2026-08-24-add-shared-session-project-memory-capture/proposal.md) — verified implementation，已同步 `shared-session-project-memory` / `project-memory-auto-capture`；Shared V2 committed 路径补输入采集 + 完成融合，项目记忆整轮入库
- [`2026-08-24-add-collab-first-stage-main-canvas-context`](2026-08-24-add-collab-first-stage-main-canvas-context/proposal.md) — verified implementation，已同步 `collab-first-stage-main-canvas-context` / `multi-agent-orchestration`；协作首节点注入主幕 digest，「按上面做」不再冷启动
- [`2026-08-24-add-multi-agent-inspector-inject-context`](2026-08-24-add-multi-agent-inspector-inject-context/proposal.md) — verified implementation，已同步 `multi-agent-inspector-inject-context` / `multi-agent-orchestration`；Inspector 顶部展示节点注入上下文 Header（用户任务/批准补充/上游产出/本环节指令）
- [`2026-08-24-fix-native-parallel-provider-model-isolation`](2026-08-24-fix-native-parallel-provider-model-isolation/proposal.md) — verified implementation，已同步 `claude-provider-runtime-model-sync` / `engine-per-session-provider-binding`；跨供应商 residual 启发式扩展 + catalog 合法集优先，并行会话历史不再串 model（MiniMax-M3 打 DeepSeek 实证）；基石 ADR 校准已回写
- [`2026-08-24-fix-native-followup-engine-collision`](2026-08-24-fix-native-followup-engine-collision/proposal.md) — verified implementation，已同步 `native-followup-engine-collision`；DSH/任意 native 续聊不得因同名 runtime 下崽，本 catalog `id`/`.model` 优先；用户手测通过
- [`2026-08-24-fix-deepseek-balance-quota`](2026-08-24-fix-deepseek-balance-quota/proposal.md) — verified implementation，已同步 `provider-balance-quota` / `status-panel-session-overview`；DeepSeek 官方余额 API 接入，Native/Shared 双路径额度展示
- [`2026-08-24-fix-grok-kimi-native-memory-completion`](2026-08-24-fix-grok-kimi-native-memory-completion/proposal.md) — verified implementation，已同步 `project-memory-auto-capture`；Kimi/Grok 流式 TurnCompleted 补 item/completed 对齐，AI 回复记忆完整入库
- [`2026-08-24-fix-askuserquestion-settlement-tombstone`](2026-08-24-fix-askuserquestion-settlement-tombstone/proposal.md) — verified implementation，已同步 `codex-chat-canvas-user-input-elicitation`；session 级 settlement tombstone + 成功应答 emit `completed=true`，幽灵重弹根除；用户 2026-08-12 验收通过；基石 ADR 校准已回写
- [`2026-08-24-add-dsh-engine`](2026-08-24-add-dsh-engine/proposal.md) — verified implementation，已同步 `dsh-cli-lifecycle` / `dsh-engine-runtime` / `dsh-session-history`；第 7 个 Native Engine DSH（`dsh-host-rpc` + 全局 `dsh web` supervisor）；失效 `engine-capability-matrix` delta 未随档（主 spec 已重构为生成物治理，dsh 行已在生成矩阵）；基石 ADR 已含校准
- [`2026-08-24-adapt-dsh-goal-continuation`](2026-08-24-adapt-dsh-goal-continuation/proposal.md) — verified implementation，已同步 `dsh-goal-continuation`；Goal `active` 抑制 `TurnCompleted`、completed hop 不解绑、`source.kind=goal` 投影 `dsh-goal` 折叠卡
- [`2026-08-24-wire-dsh-auto-mode-permission`](2026-08-24-wire-dsh-auto-mode-permission/proposal.md) — verified implementation，已同步 `dsh-auto-mode-permission` / `dsh-engine-runtime`；DSH auto mode 映射 `danger-full-access` preset，退出回 `workspace-write`，busy/queued turn 跳过 `/permission`
- [`2026-08-24-fix-dsh-custom-route-image-admission`](2026-08-24-fix-dsh-custom-route-image-admission/proposal.md) — verified implementation，已同步 `dsh-engine-runtime`；附图自动给 writable `llm-pi-ai` 路由声明 `[text, image]`，不再要求手改 `settings.yaml`
- [`2026-08-24-show-dsh-session-stats`](2026-08-24-show-dsh-session-stats/proposal.md) — verified implementation，已同步 `dsh-session-stats` / `dsh-session-history`；DSH host `sessionStats`/`tokenUsage` 投影 composer 展示
- [`2026-08-24-add-dsh-agent-preset-picker`](2026-08-24-add-dsh-agent-preset-picker/proposal.md) — verified implementation，已同步 `dsh-agent-preset-picker` / `dsh-engine-runtime`；DSH create-time `agentPreset` 传递 + 历史 preset 展示
- [`2026-08-24-harden-dsh-npm-global-install`](2026-08-24-harden-dsh-npm-global-install/proposal.md) — verified implementation，已同步 `dsh-cli-lifecycle`；DSH 一键安装 npm 命令 / timeout / 重试文案加固
- [`2026-08-24-add-qoder-dual-distribution`](2026-08-24-add-qoder-dual-distribution/proposal.md) — verified implementation，已同步 `qoder-dual-distribution`、`engine-per-session-provider-binding` 等 4 capability；Qoder Global/CN 单 engine 双分发，`providerProfileId` 升 immutable distribution binding，Tx1 未知 profile fail-closed；基石 ADR 校准已回写
- [`2026-08-24-enhance-pi-native-rpc-session`](2026-08-24-enhance-pi-native-rpc-session/proposal.md) — verified implementation，已同步 `pi-rpc-session-runtime` / `pi-session-fork-tree` / `composer-queued-followup-fusion`；PI 迁移 `pi --mode rpc` 长驻进程（resident per workspace×session 真并行、`steer` 续聊、看门狗对账结算、fork/tree/compact、图片 base64 `images[]`）；失效 `engine-capability-matrix` delta 未随档（生成矩阵已含 pi 行）；基石 ADR 已含校准
- [`2026-08-24-add-first-run-setup-wizard`](2026-08-24-add-first-run-setup-wizard/proposal.md) — verified implementation，已同步 `first-run-setup`；首次运行设置向导 + skip banner + 设置重跑
- [`2026-08-24-add-workspace-wallpaper`](2026-08-24-add-workspace-wallpaper/proposal.md) — verified implementation，已同步 `workspace-wallpaper`；工作台页面背景（默认流体/关闭/自定义上传）
- [`2026-08-24-add-fluid-motion-presets`](2026-08-24-add-fluid-motion-presets/proposal.md) — verified implementation，已同步 `workspace-wallpaper`；流体壁纸结构化动势预设（太极/暴风雨/龙卷风）+ 低饱和深灰白
- [`2026-08-24-add-workspace-sidebar-settings-dialog`](2026-08-24-add-workspace-sidebar-settings-dialog/proposal.md) — verified implementation，已同步 `workspace-sidebar-settings` / `workspace-sidebar-visual-harmony`；全局工作区设置弹窗，侧栏默认露出改 5 条
- [`2026-08-24-fold-sidebar-pinned-by-calendar-day`](2026-08-24-fold-sidebar-pinned-by-calendar-day/proposal.md) — verified implementation，已同步 `sidebar-pinned-calendar-fold`；置顶区按日历日最外层分组，默认只开最新日
- [`2026-08-24-add-windows-sidebar-quick-switcher-and-collapse`](2026-08-24-add-windows-sidebar-quick-switcher-and-collapse/proposal.md) — verified implementation，已同步 `windows-sidebar-chrome-parity`；Win 主导航 Ctrl+E Quick Switcher + 设置菜单隐藏侧栏；手测通过
- [`2026-08-24-add-windows-main-window-close-confirm`](2026-08-24-add-windows-main-window-close-confirm/proposal.md) — verified implementation，已同步 `windows-main-window-close-confirm`；Win 自绘 X 每次自定义二次确认，Mac/菜单路径不动；Win 冒烟验收通过
- [`2026-08-24-open-with-cross-platform-ux`](2026-08-24-open-with-cross-platform-ux/proposal.md) — verified implementation，已同步 `open-with-targets-ux`；打开方式跨平台 UX：预设/Browse/懒探测/系统图标；用户验收通过
- [`2026-08-24-redesign-composer-session-control-hud`](2026-08-24-redesign-composer-session-control-hud/proposal.md) — verified implementation，已同步 `composer-session-control-hud` / `composer-tool-menu-primary-controls`；会话控制升级 Metrics HUD 双栏
- [`2026-08-24-redesign-composer-prompt-enhancer`](2026-08-24-redesign-composer-prompt-enhancer/proposal.md) — verified implementation，已同步 `composer-prompt-enhancer`；增强提示词并排对照 + ModelSelect + 强度三档
- [`2026-08-24-show-live-working-indicator-tokens`](2026-08-24-show-live-working-indicator-tokens/proposal.md) — verified implementation，已同步 `conversation-working-indicator-live-tokens`；响应中条显示 live compact tokens（`4m 21s · 5.6k tokens` 形态）
- [`2026-08-24-fix-process-phase-contiguous-segmentation`](2026-08-24-fix-process-phase-contiguous-segmentation/proposal.md) — verified implementation，已同步 `message-process-phase-collapse`；过程折叠改回按段连续 walk-back，不再整轮并进终稿一个 chip
- [`2026-08-24-fold-background-task-notification`](2026-08-24-fold-background-task-notification/proposal.md) — verified implementation，已同步 `background-task-notification-fold` / `shared-message-domain-helpers`；Claude 后台 Bash wakeup 折叠，不再冒充用户蓝气泡
- [`2026-08-24-fix-assistant-duplicate-render-native-shared`](2026-08-24-fix-assistant-duplicate-render-native-shared/proposal.md) — verified implementation，已同步 `assistant-duplicate-render-convergence` 等 5 capability；assistant 单气泡 A2+A 回显 + 跨 id 双气泡收敛
- [`2026-08-24-fix-composer-rewind-reset-update-depth-loop`](2026-08-24-fix-composer-rewind-reset-update-depth-loop/proposal.md) — verified implementation，已同步 `client-renderer-stability-under-pressure`（主 spec 已含同等文本，archive --skip-specs）；rewind reset 跳过冗余 React dispatch，修 React #185 整窗接管
- [`2026-08-24-retire-claude-subagent-agent-session-card`](2026-08-24-retire-claude-subagent-agent-session-card/proposal.md) — verified implementation，已同步 `claude-subagent-canvas-surface` 等 3 capability；退役 SubAgent 旧 Agent session 卡，事实迁 S10/inspector
- [`2026-08-24-session-directory-grant`](2026-08-24-session-directory-grant/proposal.md) — verified implementation，已同步 `session-directory-grant` 等 3 capability；workspace 外路径授权卡 + L1 allowlist 运行时扩权（issue #1062）
- [`2026-08-24-fix-custom-prompts-stale-empty-cache`](2026-08-24-fix-custom-prompts-stale-empty-cache/proposal.md) — verified implementation，已同步 `composer-prompt-completion`；`!` 提示词 soft-failure 永久空态修复：soft-cancel 保留缓存 + 空态 on-demand revalidate
- [`2026-08-24-hide-startup-gate-overlay`](2026-08-24-hide-startup-gate-overlay/proposal.md) — verified implementation，已同步 `client-startup-orchestration`；StartupGateOverlay 默认隐藏、local opt-in、next-start-only
- [`2026-08-24-custom-model-provider-binding`](2026-08-24-custom-model-provider-binding/proposal.md) — verified implementation，已同步 `vendor-custom-model-provider-binding` 等 5 capability；自定义模型录入先选供应商，回写 managed provider `customModels`
- [`2026-08-24-perf-large-transcript-first-paint`](2026-08-24-perf-large-transcript-first-paint/proposal.md) — verified implementation，已同步 `large-transcript-first-paint`；大 transcript 首屏 hydrate / window cut / 翻页性能
- [`2026-08-24-defer-noncritical-bootstrap-io`](2026-08-24-defer-noncritical-bootstrap-io/proposal.md) — verified implementation，已同步 `client-startup-orchestration`；bootstrap critical store/locale 拆分，非关键 IO 延后
- [`2026-08-24-defer-thread-list-hydration-until-idle-or-intent`](2026-08-24-defer-thread-list-hydration-until-idle-or-intent/proposal.md) — verified implementation，冷启 thread list 改 idle-or-intent 触发；delta 锚定的 `client-startup-orchestration` / `runtime-workspace-switch-hydration` 目标已被后续冷启线重构（仍有 active change 收尾），archive --skip-specs，行为合同由主 spec 与后续 change 承接
- [`2026-08-24-optimize-ccgui-relationship-io`](2026-08-24-optimize-ccgui-relationship-io/proposal.md) — verified implementation，已同步 `project-map-relationship-io`；关系快照按需读取、Search Radar 停止偷扫、备份当过期缓存回收
- [`2026-08-24-fix-unstaged-discard-preserve-index`](2026-08-24-fix-unstaged-discard-preserve-index/proposal.md) — verified implementation，已同步 `git-panel-diff-view`；未暂存 discard 只 `restore --worktree`，staged 内容不再丢失（P0 数据损失修复）
