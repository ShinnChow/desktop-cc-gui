...
async fn build_workspace_scope_catalog_data(
    ...
    for workspace in &workspace_scope {
        ...
        match engine::pi_history::list_pi_family_sessions(
        ...
        .await
        {
            Ok(omp_sessions) => {
                ...
                entries.extend(omp_sessions.into_iter().map(|session| {
                    let session_id = format!("omp:{}", session.session_id);
                    let entry = WorkspaceSessionCatalogEntry {
                        archived_at: archived_at_for_session(
                            &owner_metadata,
                            &owner_workspace_id,
            ...
            Err(error) => {
                log::warn!(
                    "[session_management.list_workspace_sessions] omp history unavailable for workspace {}: {}",
                    owner_workspace_id,
                    error
                );
                partial_sources.push(SESSION_CATALOG_PARTIAL_OMP.to_string());
                source_statuses.push(build_degraded_source_status(
                    "omp",
                    SESSION_CATALOG_PARTIAL_OMP,
                ));
            }
        }
