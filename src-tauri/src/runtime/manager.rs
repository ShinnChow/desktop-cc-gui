...
use super::consts::{
    LEDGER_FILE_NAME, RUNTIME_ACQUIRE_WAIT_TIMEOUT_SECS,
    RUNTIME_RECOVERY_MAX_CONSECUTIVE_FAILURES, RUNTIME_RECOVERY_RETRY_BACKOFF_MILLIS,
    RUNTIME_RECOVERY_QUARANTINE_MILLIS, THREAD_CREATE_PENDING_SENTINEL,
};
use super::coordinator::{EvictionCandidate, RuntimeLifecycleCoordinator};
use super::end_context::RecentRuntimeEndContext;
use super::entry::{now_millis, RuntimeEntry};
use super::event_sources::{
    event_method, event_stream_source, event_thread_id, event_turn_id, event_turn_source,
};
use super::executable_registry::{ExecutableSessionRegistry, ExecutableSessionState};
use super::gates::{
    RuntimeAcquireDisposition, RuntimeAcquireGate, RuntimeAcquireGateEntry, RuntimeAcquireToken,
    RuntimeReplacementGate, RuntimeReplacementGateEntry, RuntimeReplacementToken,
};
use super::identity::{normalize_engine, runtime_key};
use super::pool_types::{
    runtime_pool_summary_from_rows, RuntimeEndedRecord, RuntimeForegroundWorkState,
    RuntimeLifecycleState, RuntimePoolBudgetSnapshot, RuntimePoolDiagnostics, RuntimePoolRow,
    RuntimePoolSnapshot, RuntimeStartupState, RuntimeState, TurnReconciliationRuntimeStatus,
...
use super::session_lifecycle::terminate_workspace_session_with_source;
...
pub(crate) struct RuntimeManager {
    ...
    pub(super) startup_gates: Mutex<HashMap<String, RuntimeAcquireGateEntry>>,
    ...
    pub(super) executable_registry: Arc<ExecutableSessionRegistry>,
...
impl RuntimeManager {
    ...
    pub(super) async fn begin_runtime_acquire_or_retry_with_timeout(
        &self,
        engine: &str,
        workspace_id: &str,
        source: &str,
    ...
    pub(super) async fn begin_runtime_replacement(
        &self,
        engine: &str,
        workspace_id: &str,
    ...
    pub(super) async fn finish_runtime_replacement(&self, token: &RuntimeReplacementToken) {
        let notify = {
            let mut replacement_gates = self.replacement_gates.lock().await;
    ...
    pub(super) fn classify_state(entry: &RuntimeEntry) -> RuntimeState {
