...
interface ModelSelectProps {
  ...
  onOpenCliSettings?: (highlightTarget?: QoderSettingsHighlightTarget) => void;
...
export const ModelSelect = memo(
  ({
    ...
    >({});
    const [refreshConfigError, setRefreshConfigError] = useState<string | null>(
      null,
    );
    ...
    const currentModel = hasTargetGroups
      ...
      : selectedModelValue.length > 0
        ? (effectiveModels.find((model) => model.id === selectedModelValue) ??
          null)
        : null;
    ...
    const qoderSettingsHighlightTarget:
      ...
      <button
        className={
          triggerVariant === "readiness"
            ? "composer-readiness-target composer-readiness-target-button"
            : "selector-button"
        }
        title={
          triggerReady
    ...
    const menu = (
      <DropdownMenu open={isOpen} onOpenChange={handleMenuOpenChange}>
        ...
        <DropdownMenuContent
          ...
          className={
            menuLayer === "overlay"
              ? "prompt-enhancer-model-menu max-h-[380px] w-64 overflow-y-auto"
          ...
          {hasPickerGroups ? (
            <>
              ...
              {pickerGroups.map((group, groupIndex) => {
                ...
                const canAddModel =
                  Boolean(onAddModel) && group.providerId !== "dsh";
                return (
                  <Fragment key={group.providerId}>
                    ...
                    <DropdownMenuSub>
                      <DropdownMenuSubTrigger
                        ...
                        data-selected={
                          isGroupCurrent(group) ? "true" : undefined
                        }
                      ...
                      <DropdownMenuSubContent
                        ...
                        className={
                          menuLayer === "overlay"
                            ? "prompt-enhancer-model-menu max-h-[380px] w-72 overflow-y-auto"
                            : "max-h-[380px] w-72 overflow-y-auto"
                        }
                      >
                        <DropdownMenuLabel className="flex items-center gap-1.5 text-muted-foreground">
                          ...
                          {groupRefresh && (
                            <button
                              ...
                              aria-label={t(
                                groupRefresh.spinning
                                  ? "models.refreshingConfig"
                        ...
                        {refreshConfigError &&
                          ...
                          group.providerId === currentProvider && (
                            <div
                              ...
                              role="status"
                            >
                              {t("models.refreshConfigFailed", {
                                message: refreshConfigError,
                              })}
                            </div>
                          )}
                      </DropdownMenuSubContent>
