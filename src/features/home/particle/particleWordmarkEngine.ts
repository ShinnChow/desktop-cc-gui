/*!
 * Copyright (c) 2023 Lorenzo. MIT License.
 * Adapted from First Light (Moyf). Full notice: ./LICENSE.first-light
 */
/**
 * Particle wordmark engine — ported from the First Light Obsidian plugin
 * (https://github.com/Moyf/first-light), MIT licensed reusable core.
 *
...
const PARTICLE_BLEED = 32 // Room for cursor displacement outside the glyph box.
...
export class ParticleWordmarkEngine {
    ...
    private hiddenElements: { element: HTMLElement; previousOpacity: string }[] = []
    ...
    private readonly handleMouseMove = (event: MouseEvent): void => {
        ...
        this.mouse.x = (event.clientX - rect.left) * this.cssWidth / rect.width
        this.mouse.y = (event.clientY - rect.top) * this.cssHeight / rect.height
        this.startLoop()
    }
    ...
    private readonly handleResize = (): void => {
        ...
        this.resizeTimer = window.setTimeout(() => {
            ...
            if (Math.abs(rect.width - this.contentWidth) < 1 && Math.abs(rect.height - this.contentHeight) < 1
                && this.scale === Math.min(3, Math.max(2, window.devicePixelRatio || 1))) return
    ...
    async build(): Promise<boolean> {
        ...
        for (const op of sources.ops) {
            ...
            try {
                ...
                console.warn('[ccgui] Particle effect: a wordmark source could not be rasterized.', error)
                return false
            }
        }
        if (this.destroyed || token !== this.buildToken) return false
        ...
        this.cssWidth = containerRect.width * this.zoom + PARTICLE_BLEED * 2
        this.cssHeight = containerRect.height * this.zoom + PARTICLE_BLEED * 2

        try {
            this.particles = this.sampleParticles(offscreen, offscreenContext, scale)
        } catch (error) {
    ...
    private async resample(): Promise<void> {
        ...
        const offscreenContext = offscreen.getContext('2d')
        if (!offscreenContext) { this.destroy(); return }
        offscreenContext.scale(scale, scale)
        ...
        for (const op of this.collectSources(contentRect).ops) {
            ...
            try {
                ...
                console.warn('[ccgui] Particle effect: a wordmark source could not be rasterized.', error)
                if (token === this.buildToken) this.destroy()
                return
            }
        }
        if (this.destroyed || token !== this.buildToken) return
        ...
        try {
            const particles = this.sampleParticles(offscreen, offscreenContext, scale)
            if (this.destroyed || token !== this.buildToken || !this.canvas || !this.renderContext) return
            if (particles.length === 0) { this.destroy(); return }
            this.scale = scale
            this.contentWidth = contentRect.width
            this.contentHeight = contentRect.height
            this.cssWidth = contentRect.width * this.zoom + PARTICLE_BLEED * 2
            this.cssHeight = contentRect.height * this.zoom + PARTICLE_BLEED * 2
    ...
    private collectSources(containerRect: { left: number; top: number }): CapturedSources {
            ...
            if (logoContainer) {
                ...
                if (target) {
                    ...
                    if (rect.width > 0 && rect.height > 0) {
                        const offsetX = (rect.left - containerRect.left) / this.domScale
                        const offsetY = (rect.top - containerRect.top) / this.domScale
                        if (svg) {
                            ops.push({ kind: 'svg', element: svg, offsetX, offsetY, width: rect.width / this.domScale, height: rect.height / this.domScale })
                        } else if (img) {
                            ops.push({ kind: 'image', element: img, offsetX, offsetY, width: rect.width / this.domScale, height: rect.height / this.domScale })
                        }
        ...
        if (this.options.titleSelector) {
            ...
            if (heading && heading.textContent && heading.textContent.trim().length > 0) {
                ...
                if (rect.width > 0 && rect.height > 0) {
                    ops.push({ kind: 'text', element: heading, offsetX: (rect.left - containerRect.left) / this.domScale, offsetY: (rect.top - containerRect.top) / this.domScale })
    ...
    private async applyDrawOp(context: CanvasRenderingContext2D, op: DrawOp): Promise<void> {
        if (op.kind === 'text') {
            ...
            const fit = Math.min(op.width / op.element.naturalWidth, op.height / op.element.naturalHeight)
            const width = op.element.naturalWidth * fit
            const height = op.element.naturalHeight * fit
            context.drawImage(op.element, op.offsetX + (op.width - width) / 2, op.offsetY + (op.height - height) / 2, width, height)
        } else {
    ...
    private drawText(context: CanvasRenderingContext2D, element: HTMLHeadingElement, offsetX: number, offsetY: number): void {
        ...
        // DOM ranges preserve wrapping, CJK/emoji graphemes, alignment and letter spacing.
        for (let node = walker.nextNode(); node; node = walker.nextNode()) {
            for (const { segment, index } of segmenter.segment(node.textContent ?? '')) {
                range.setStart(node, index)
                range.setEnd(node, index + segment.length)
                const rect = range.getBoundingClientRect()
                const x = offsetX + (rect.left - elementRect.left) / this.domScale
                const y = offsetY + (rect.top - elementRect.top) / this.domScale
                context.fillText(segment, x, y + (rect.height / this.domScale - ascent - descent) / 2 + ascent)
            }
        }
    }
    ...
    private sampleParticles(offscreen: HTMLCanvasElement, context: CanvasRenderingContext2D, scale: number): Particle[] {
        ...
        let step = Math.max(1, Math.round(this.options.spacing * scale)) // integer device pixels, including fractional DPR
        let particles = this.collectParticles(data, offscreen.width, offscreen.height, step, mono, scale)
        while (particles.length > MAX_PARTICLES) {
            step *= 2
            particles = this.collectParticles(data, offscreen.width, offscreen.height, step, mono, scale)
        }
        return particles
    }

    private collectParticles(data: Uint8ClampedArray, width: number, height: number, step: number, mono: RGB | null, scale: number): Particle[] {
        ...
        for (let y = Math.floor(step / 2); y < height; y += step) {
            for (let x = Math.floor(step / 2); x < width; x += step) {
                ...
                const hx = (x / scale) * this.zoom + PARTICLE_BLEED
                const hy = (y / scale) * this.zoom + PARTICLE_BLEED
                let fill: string
                if (mono) {
    ...
    private activate(sources: CapturedSources): boolean {
        if (!this.installCanvas()) return false
        this.render()
    ...
    private resolveContentRect(): DOMRect {
        const content = this.options.contentSelector
            ...
            : null
        const rect = (content ?? this.container).getBoundingClientRect()
        const outer = this.container.getBoundingClientRect()
        this.domScale = this.container.offsetWidth > 0 ? outer.width / this.container.offsetWidth : 1
        if (!this.domScale) this.domScale = 1
        return new DOMRect(rect.left, rect.top, rect.width / this.domScale, rect.height / this.domScale)
    }

    private installCanvas(): boolean {
    ...
    /** Opacity preserves layout AND the accessible heading; canvas is decorative. */
    private hideCapturedElements(elements: HTMLElement[]): void {
        this.hiddenElements = elements.map((element) => {
            ...
            return { element, previousOpacity }
        })
    }

    private restoreCapturedElements(): void {
        for (const { element, previousOpacity } of this.hiddenElements) {
    ...
    private startLoop(): void {
        if (this.destroyed || this.rafId !== null || document.hidden) return
        const frame = (): void => {
            this.rafId = null
            if (this.destroyed) return
            ...
            this.render()
            if (moving) this.rafId = window.requestAnimationFrame(frame)
        }
    ...
    private step(): boolean {
        ...
        for (const particle of this.particles) {
            ...
            if (Math.abs(particle.vx) + Math.abs(particle.vy) > 0.02) moving = true
            if (distanceSquared >= radiusSquared && Math.abs(particle.hx - particle.x) + Math.abs(particle.hy - particle.y) > 0.1) moving = true
        }
        return moving
    }
