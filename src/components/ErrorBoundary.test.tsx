...
describe("ErrorBoundary react-scan recovery", () => {
  ...
  it("records the bounded recovery and skips the ordinary error detail update", () => {
    ...
    expect(recoverFromReactScanUpdateDepthErrorMock).toHaveBeenCalledWith(
      error,
    );
  ...
  it("attaches structured attribution fields to the ordinary error diagnostic", () => {
    // F1（enhance-perf-diagnostics-evidence）：errorName/文本指纹/组件帧安全命名落盘，
    // 供本地诊断定位崩溃组件（error/componentStack 本体仍由 sanitize 脱敏）。
    recoverFromReactScanUpdateDepthErrorMock.mockReturnValue("not-applicable");
    const consoleErrorMock = vi
      .spyOn(console, "error")
      .mockImplementation(() => undefined);
...
describe("ErrorBoundary crash UI", () => {
  ...
  it("shows decoded React #185, component stack, copy and feedback actions", async () => {
    ...
    expect(
      screen.getByTestId("application-error-message").textContent,
    ).toContain("Minified React error #185");
    expect(
      screen.getByTestId("application-error-decoded").textContent,
    ).toContain("Maximum update depth exceeded");
    // componentDidCatch fills component stack asynchronously via setState
    await waitFor(() => {
      expect(
        screen.getByTestId("application-error-component-stack").textContent,
      ).toContain("at Boom");
    });
    ...
    expect(copied).toContain(
      "github.com/zhukunpenglinyutong/desktop-cc-gui/issues",
    );
