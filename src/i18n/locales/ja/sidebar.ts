// AUTO-GENERATED by scripts/i18n/build-locale.ts — do not edit by hand.
// Machine translation of src/i18n/locales/en/sidebar.ts; keys mirror the English source.
// AUTO-GENERATED by scripts/i18n/build-locale.ts — do not edit by hand.
// Machine translation of src/i18n/locales/en/sidebar.ts; keys mirror the English source.
const sidebar = {
  "quickSwitcher": {
    "open": "最近のアクティビティ",
    "title": "最近のアクティビティ",
    "navigation": "クイックナビゲーション",
    "recentSessions": "最近のセッション",
    "runningSessions": "実行中",
    "recentFiles": "最近のファイル",
    "emptySessions": "最近のセッションはありません",
    "emptyFiles": "最近開いた、または編集したファイルはありません",
    "aiModified": "AI が編集",
    "keyboardHint": "移動",
    "switchPaneHint": "ペイン切替",
    "openHint": "開く",
    "closeHint": "閉じる",
    "hints": {
      "selectWorkspaceFirst": "この機能を開くには、先にワークスペースを選択してください",
    },
    "nav": {
      "globalSearch": "全体検索",
      "chat": "チャット",
      "files": "ファイル",
      "git": "変更",
      "history": "Git 履歴",
      "spec": "Spec Hub",
      "intentCanvas": "インテントキャンバス",
      "projectMap": "プロジェクトマップ",
      "terminal": "ターミナル",
      "notes": "メモ",
      "memory": "プロジェクトメモリ",
      "settings": "設定",
    },
  },
  "sidebar": {
    "projects": "プロジェクト",
    "mcpSkillsMarket": "MCP & Skills マーケット",
    "longTermMemory": "長期メモリ",
    "pluginMarket": "プラグインマーケット",
    "plugins": "プラグイン",
    "specHub": "Spec Hub",
    "comingSoon": "近日公開",
    "comingSoonMessage": "この機能は開発中です。お楽しみに",
    "cliComingSoon": "近日公開",
    "cliNotInstalled": "未インストール",
    "addWorkspace": "プロジェクトを追加",
    "workspaceSettings": "ワークスペース設定",
    "workspaceSettingsTitle": "ワークスペース設定",
    "workspaceSettingsDescription": "これらの設定はすべてのプロジェクトに適用されます。",
    "workspaceSettingsVisibleCountLabel": "既定の表示セッション数",
    "workspaceSettingsVisibleCountHint": "サイドバー折りたたみ時に既定で表示するルートセッション数です。初期値 {{defaultCount}}、範囲 {{min}}-{{max}}。プロジェクト単位ではセッション管理で上書きできます。",
    "openHome": "ホームを開く",
    "toggleSearch": "検索を切り替え",
    "searchProjects": "プロジェクトを検索",
    "clearSearch": "検索をクリア",
    "pinned": "ピン留め",
    "pinnedCount": "· {{count}}",
    "collapsePinnedSection": "ピン留めを折りたたむ",
    "expandPinnedSection": "ピン留めを展開",
    "collapsePinnedDay": "{{date}} を折りたたむ",
    "expandPinnedDay": "{{date}} を展開",
    "newAgent": "新しい Agent",
    "newSharedSession": "Shared CLI",
    "nativeCliGroupLabel": "Native CLI",
    "sharedCliHint": "エンジン切替可",
    "nativeCliHint": "エンジン固定",
    "sharedCliHelp": "複数CLIが同じコンテキストを共有し、途中でエンジンを切替可能",
    "nativeCliHelp": "各セッションは単一エンジンに固定され、コンテキストは独立",
    "providerChoiceHelp": "選択で記憶：このCLIの新規セッションは既定でそれを使用。「公式設定に従う」は起動設定に連動、「独立設定」はアカウント独立。設定 → CLI設定管理 → プロバイダーチャンネル で管理",
    "newWorktreeAgent": "新しい Worktree Agent",
    "newCloneAgent": "新しいクローン Agent",
    "codexProviderChoiceTitle": "プロバイダーの選択",
    "codexProviderSelectedTip": "選択しました。左側の Codex をクリックしてセッションを開始します。",
    "grokProviderChoiceTitle": "プロバイダーの選択",
    "grokProviderSelectedTip": "選択しました。左側の Grok CLI をクリックしてセッションを開始します。",
    "opencodeProviderChoiceTitle": "プロバイダーの選択",
    "opencodeProviderSelectedTip": "選択しました。左側の OpenCode をクリックしてセッションを開始します。",
    "providerFollowsGlobalLabel": "グローバル設定に従う",
    "providerIsolatedConfigLabel": "独立設定",
    "sessionActionsGroup": "新しいセッション",
    "newSessionInFolder": "{{name}} で新しいセッション",
    "activateWorkspace": "メインパネルで開く",
    "workspaceActionsGroup": "ワークスペースの操作",
    "setWorkspaceAlias": "エイリアスを設定",
    "assignWorkspaceGroup": "プロジェクトグループを変更",
    "workspaceAliasPrompt": "サイドバーでこのワークスペースに表示するエイリアスを入力します。空欄のままにすると元のプロジェクト名が使われます。",
    "workspaceAliasDialogTitle": "ワークスペースのエイリアスを設定",
    "workspaceAliasDialogSubtitle": "元のプロジェクト名: <code>{{name}}</code>",
    "workspaceAliasLabel": "サイドバーの表示名",
    "workspaceAliasPlaceholder": "例: 請求バックエンド",
    "workspaceAliasEmptyHint": "空の値で保存するとエイリアスがクリアされ、元のプロジェクト名が表示されます。",
    "workspaceAliasBadge": "A",
    "workspaceAliasBadgeTitle": "ワークスペースのエイリアス。元の名前: {{name}}",
    "emptyWorkspaceSessions": "セッションはまだありません。",
    "loadingWorkspaceSessions": "読み込み中…",
    "loadingWorkspaceSessionsIndex": "セッションインデックスを読み込み中…",
    "loadingWorkspaceSessionsDeep": "ローカルセッションを完全スキャン中…",
    "newSessionFolder": "新しいフォルダ",
    "newSessionFolderIn": "{{name}} に新しいフォルダ",
    "renameSessionFolder": "{{name}} の名前を変更",
    "deleteSessionFolder": "{{name}} を削除",
    "sessionFolderActions": "{{name}} フォルダの操作",
    "collapseSessionFolder": "{{name}} を折りたたむ",
    "expandSessionFolder": "{{name}} を展開",
    "sessionFolderContextMenuPrompt": "名前変更・削除・新規・折りたたみ・展開のいずれかを入力してください。",
    "sessionFolderNamePrompt": "フォルダ名",
    "sessionFolderRenamePrompt": "フォルダの名前を変更",
    "sessionFolderDeleteTitle": "フォルダを削除",
    "sessionFolderDeleteMessage": "「{{name}}」を削除してもよろしいですか？",
    "sessionFolderDeleteHint": "中身のあるフォルダは先に空にする必要があります。この操作は取り消せません。",
    "sessionFolderCreateFailed": "フォルダを作成できませんでした",
    "sessionFolderRenameFailed": "フォルダの名前を変更できませんでした",
    "sessionFolderDeleteFailed": "フォルダを削除できませんでした",
    "sessionFolderMoveFailed": "セッションを移動できませんでした",
    "loadingProgressMoveSessionTitle": "セッションを移動中...",
    "loadingProgressMoveSessionMessage": "このセッションを {{folder}} に移動しています。",
    "sessionFolderCrossProjectBlocked": "セッションはプロジェクトをまたいで移動できません。",
    "sessionFolderCount": "{{count}} 件のセッション",
    "sessionFolderLoadFailed": "セッションフォルダを利用できません。",
    "sessionFolderDropToRoot": "ここにドロップしてプロジェクトのルートへ移動",
    "sessionFolderDropBlocked": "このセッションは別のプロジェクトに属しています",
    "deprecatedTag": "非推奨",
    "unavailableTag": "利用不可",
    "removeWorkspace": "ワークスペースを削除",
    "noProjectsMatch": "検索に一致するプロジェクトがありません。",
    "addWorkspaceToStart": "始めるにはワークスペースを追加してください。",
    "dropProjectHere": "ここにプロジェクトをドロップ",
    "addingProject": "プロジェクトを追加中...",
    "apiKey": "API キー",
    "signInToCodex": "Codex にサインイン",
    "switchAccount": "アカウントを切り替え",
    "signIn": "サインイン",
    "account": "アカウント",
    "session": "セッション",
    "currentSession": "現在のセッション",
    "expandCurrentSession": "現在のセッションを展開",
    "collapseCurrentSession": "現在のセッションを折りたたむ",
    "expandAllSections": "すべてのセクションを展開",
    "collapseAllSections": "すべてのセクションを折りたたむ",
    "weekly": "週間",
    "openDebugLog": "デバッグログを開く",
    "debugLog": "デバッグログ",
    "hideThreadsSidebar": "スレッドサイドバーを非表示",
    "showThreadsSidebar": "スレッドサイドバーを表示",
    "hideGitSidebar": "右サイドバーを非表示",
    "showGitSidebar": "右サイドバーを表示",
    "enterSoloMode": "SOLO モードに入る",
    "exitSoloMode": "SOLO モードを終了",
    "quickNewThread": "ホーム",
    "quickSearch": "検索",
    "quickSkills": "Skills",
    "releaseNotes": "リリースノート",
    "threadsSection": "スレッド"
  },
  "extensions": {
    "usage": {
      "checkingLabel": "tokentracker-cli を確認しています…",
      "installingLabel": "tokentracker-cli をインストールしています…",
      "installingDesc": "少し時間がかかる場合があります。インストール後、アプリが再検出してローカルサーバーを起動します。",
      "startingLabel": "tokentracker サーバーを起動しています…",
      "guideTitle": "使用統計を見るには tokentracker-cli をインストール",
      "guideDesc": "使用統計は tokentracker-cli が提供します。ローカルサーバーを起動し、各 AI ツールのトークン使用量を集約してダッシュボードに表示します。",
      "guideInstallLabel": "インストールコマンド",
      "guideInstallNow": "今すぐインストール",
      "guideCopy": "コピー",
      "guideCopied": "コピーしました",
      "guideOpenNpm": "npm ページを開く",
      "guideRecheck": "再検出",
      "guideNoteHooks": "初回起動時、tokentracker サーバーはインストール済みの AI ツールに hooks を書き込みます（これがデータ収集の仕組みです）。",
      "guideNoteTelemetry": "本アプリでは匿名テレメトリを無効化しています。",
      "errorTitle": "tokentracker サーバーの起動に失敗しました",
      "errorRetry": "再試行"
    },
    "mcps": {
      "title": "MCPs",
      "refresh": "更新",
      "tabs": {
        "claude": "Claude Code",
        "codex": "Codex"
      },
      "filter": {
        "sourceLabel": "ソース",
        "sourceAll": "すべてのソース",
        "sourceConfig": "設定ファイル",
        "sourceRuntime": "ランタイム",
        "resultCount": "{{filtered}} / {{total}} サーバー",
        "clear": "フィルターをクリア"
      },
      "search": {
        "placeholder": "MCP サーバーを検索…",
        "aria": "MCP サーバーを検索",
        "clear": "検索をクリア"
      },
      "groups": {
        "config": "設定ファイル",
        "runtime": "ランタイム"
      },
      "row": {
        "openDetails": "{{name}} の詳細を開く",
        "toolsCount": "{{count}} ツール"
      },
      "badges": {
        "enabled": "有効",
        "disabled": "無効",
        "builtIn": "組み込み",
        "statusUnknown": "ステータス不明"
      },
      "toggle": {
        "ariaEnable": "{{name}} を有効にする",
        "ariaDisable": "{{name}} を無効にする",
        "failed": "{{name}} の有効状態の切り替えに失敗しました：{{message}}"
      },
      "detail": {
        "close": "詳細を閉じる",
        "sectionTitle": "サーバーの詳細",
        "source": "ソース",
        "sourceClaude": "~/.claude.json",
        "sourceCcgui": "~/.ccgui/config.json",
        "transport": "トランスポート",
        "transportUnknown": "不明なトランスポート",
        "command": "コマンド",
        "commandMeta": "{{command}} · 引数 {{args}}",
        "url": "URL",
        "urlMeta": "URL：{{url}}",
        "status": "ステータス",
        "auth": "認証",
        "tools": "ツール",
        "resourcesTemplatesLabel": "リソース / テンプレート",
        "resourcesTemplates": "リソース {{resources}} / テンプレート {{templates}}"
      },
      "empty": {
        "title": "MCP サーバーが見つかりません",
        "description": "設定ファイルに MCP サーバーを追加してから更新すると、ここに表示されます。",
        "noMatch": "現在のフィルターに一致する MCP サーバーはありません"
      }
    },
  }
};

export default sidebar;
